...
public class TXRoomService implements ITXRoomService {
    ...
    public void login(int sdkAppId, final String userId, String userSign, final TXCallback callback) {
        if (TUILogin.isUserLogined()) {
            ...
            TRTCLogger.INSTANCE.i(TAG, "already login.");
            if (callback != null) {
        ...
        TUILogin.login(userId, userSign, new V2TIMCallback() {
            ...
            public void onError(int code, String msg) {
                TRTCLogger.INSTANCE.e(TAG, "login fail code: " + code + " msg:" + msg);
                if (callback != null) {
            ...
            public void onSuccess() {
                TRTCLogger.INSTANCE.i(TAG, "login onSuccess");
                mIsLogin = true;
                mMySelfIMInfo.userId = userId;
                if (callback != null) {
                    callback.onCallback(0, "login im success.");
    ...
    public void logout(final TXCallback callback) {
        if (!isLogin()) {
            TRTCLogger.INSTANCE.e(TAG, "start logout fail, not login yet.");
            if (callback != null) {
        ...
        if (isEnterRoom()) {
            TRTCLogger.INSTANCE.e(TAG, "start logout fail, you are in room:" + mRoomId + ", please exit room before logout.");
            if (callback != null) {
        ...
        TUILogin.logout(new V2TIMCallback() {
            @Override
            public void onSuccess() {
                TRTCLogger.INSTANCE.i(TAG, "logout im success.");
                if (callback != null) {
                    callback.onCallback(0, "login im success.");
                }
            }
            ...
            public void onError(int i, String s) {
                TRTCLogger.INSTANCE.e(TAG, "logout fail, code:" + i + " msg:" + s);
                if (callback != null) {
    ...
    public void setSelfProfile(final String userName, final String avatarURL, final TXCallback callback) {
        if (!isLogin()) {
            TRTCLogger.INSTANCE.e(TAG, "set profile fail, not login yet.");
            if (callback != null) {
        ...
        V2TIMManager.getInstance().setSelfInfo(v2TIMUserFullInfo, new V2TIMCallback() {
            ...
            public void onError(int code, String desc) {
                TRTCLogger.INSTANCE.e(TAG, "set profile code:" + code + " msg:" + desc);
                if (callback != null) {
            ...
            @Override
            public void onSuccess() {
                TRTCLogger.INSTANCE.i(TAG, "set profile success.");
    ...
    public void createRoom(final String roomId, final String roomName,
        ...
        TRTCLogger.INSTANCE.e(TAG, "createRoom mIsEnterRoom:" + mIsEnterRoom);
        if (isEnterRoom()) {
            TRTCLogger.INSTANCE.e(TAG, "you have been in room:" + mRoomId + " can't create another room:" + roomId);
            if (callback != null) {
        ...
        if (!isLogin()) {
            TRTCLogger.INSTANCE.e(TAG, "not log yet, create room fail.");
            if (callback != null) {
        ...
        imManager.createGroup(V2TIMManager.GROUP_TYPE_AVCHATROOM, roomId, roomName, new V2TIMValueCallback<String>() {
            ...
            public void onError(int code, String s) {
                ...
                if (code == 10025) {
                    ...
                    TRTCLogger.INSTANCE.e(TAG, "create room fail, code:" + code + " msg:" + msg);
                    if (callback != null) {
                        callback.onCallback(code, msg);
                    }
                }
            ...
            public void onSuccess(String s) {
                imManager.joinGroup(roomId, "", new V2TIMCallback() {
                    ...
                    public void onSuccess() {
                        ...
                        TRTCLogger.INSTANCE.i(TAG, "createGroup setGroupListener roomId: " + roomId + " mGroupListener: "
                        ...
                        mRoomId = roomId;
                        getAudienceList((code, msg, list) -> {
                            if (code == 0) {
                                for (TXUserInfo userInfo : list) {
                                    mAudienceInfoMap.put(userInfo.getUserId(), userInfo);
                                }
                            }
                        });
                        ...
                        // 组装 RoomInfo 抛给上层
                        mTXRoomInfo.setRoomStatus(mCurrentRoomStatus);
                        mTXRoomInfo.setRoomId(roomId);
                        mTXRoomInfo.setRoomName(roomName);
                        mTXRoomInfo.setOwnerId(mMySelfIMInfo.userId);
                        mTXRoomInfo.setCoverUrl(coverUrl);
                        mTXRoomInfo.setOwnerName(mMySelfIMInfo.name);
                        mTXRoomInfo.setStreamUrl(mMySelfIMInfo.streamId);
                        mTXRoomInfo.setMemberCount(1);
                        ...
                        TRTCLogger.INSTANCE.i(TAG, "create room success.");
                        if (callback != null) {
    ...
    public void destroyRoom(final TXCallback callback) {
        ...
        V2TIMManager.getGroupManager().getGroupsInfo(groupList, new V2TIMValueCallback<List<V2TIMGroupInfoResult>>() {
            @Override
            public void onError(int i, String s) {
                TRTCLogger.INSTANCE.e(TAG, "room owner get group info fail, code: " + i + " msg:" + s);
                resetStatus();
            }
            ...
            public void onSuccess(List<V2TIMGroupInfoResult> v2TIMGroupInfoResults) {
                if (v2TIMGroupInfoResults != null && v2TIMGroupInfoResults.size() == 1) {
                    ...
                    V2TIMManager.getGroupManager().setGroupInfo(v2TIMGroupInfo, new V2TIMCallback() {
                        ...
                        @Override
                        public void onSuccess() {
                            TRTCLogger.INSTANCE.i(TAG, "room owner update anchor list into group introduction success");
                            V2TIMManager.getInstance().dismissGroup(mRoomId, new V2TIMCallback() {
                                @Override
                                public void onError(int i, String s) {
                                    TRTCLogger.INSTANCE.e(TAG, "destroy room fail, code:" + i + " msg:" + s);
                                    if (callback != null) {
                                        callback.onCallback(i, s);
                                    }
                                }
                                ...
                                @Override
                                public void onSuccess() {
                                    TRTCLogger.INSTANCE.i(TAG, "destroyRoom remove GroupListener roomId: "
                                            + mRoomId + " mGroupListener: " + mGroupListener.hashCode());
                                    resetStatus();
                                    TRTCLogger.INSTANCE.i(TAG, "destroy room success.");
                                    if (callback != null) {
    ...
    public void enterRoom(final String roomId, final TXCallback callback) {
        if (isEnterRoom()) {
            TRTCLogger.INSTANCE.e(TAG, "you have been in room:" + mRoomId + ", can't enter another room:" + roomId);
            if (callback != null) {
        ...
        V2TIMManager.getGroupManager().getGroupsInfo(groupList, new V2TIMValueCallback<List<V2TIMGroupInfoResult>>() {
            @Override
            public void onError(int i, String s) {
                TRTCLogger.INSTANCE.e(TAG, "get group info error, enter room fail. code:" + i + " msg:" + s);
                if (callback != null) {
            ...
            public void onSuccess(List<V2TIMGroupInfoResult> v2TIMGroupInfoResults) {
                ...
                if (v2TIMGroupInfoResults != null && v2TIMGroupInfoResults.size() == 1) {
                    ...
                    if (v2TIMGroupInfoResult != null) {
                        ...
                        TRTCLogger.INSTANCE.i(TAG, "get group info success, info:" + introduction);
                            ...
                            V2TIMManager.getInstance().joinGroup(roomId, "", new V2TIMCallback() {
                                ...
                                public void onError(int i, String s) {
                                    if (i == 10013) {
                                        ...
                                        TRTCLogger.INSTANCE.e(TAG, "enter room fail, code:" + i + " msg:" + s);
                                        if (callback != null) {
                                            callback.onCallback(i, s);
                                        }
                                    }
                                ...
                                public void onSuccess() {
                                    ...
                                    V2TIMManager.getInstance().addGroupListener(mGroupListener);
                                    V2TIMManager.getSignalingManager().addSignalingListener(mSignalingListener);
                                    TRTCLogger.INSTANCE.i(TAG, "enter room success. roomId: " + roomId);
                                    mRoomId = roomId;
                                    mIsEnterRoom = true;
                                    mTXRoomInfo.setRoomId(roomId);
                                    mTXRoomInfo.setRoomName(v2TIMGroupInfoResult.getGroupInfo().getGroupName());
                                    mTXRoomInfo.setCoverUrl(v2TIMGroupInfoResult.getGroupInfo().getFaceUrl());
                                    mTXRoomInfo.setMemberCount(v2TIMGroupInfoResult.getGroupInfo().getMemberCount());
    ...
    public void exitRoom(final TXCallback callback) {
        if (!isEnterRoom()) {
            TRTCLogger.INSTANCE.e(TAG, "not enter room yet, can't exit room.");
            if (callback != null) {
        ...
        V2TIMManager.getInstance().quitGroup(mRoomId, new V2TIMCallback() {
            @Override
            public void onError(int i, String s) {
                TRTCLogger.INSTANCE.e(TAG, "exit room fail, code:" + i + " msg:" + s);
                resetStatus();
                if (callback != null) {
                    callback.onCallback(i, s);
                }
            ...
            @Override
            public void onSuccess() {
                TRTCLogger.INSTANCE.i(TAG, "exit room success.");
        ...
        V2TIMManager.getGroupManager().getGroupsInfo(roomIds, new V2TIMValueCallback<List<V2TIMGroupInfoResult>>() {
            ...
            public void onSuccess(List<V2TIMGroupInfoResult> v2TIMGroupInfoResults) {
                ...
                for (String roomId : roomIds) {
                    ...
                    txRoomInfo.setRoomId(timGroupDetailInfo.getGroupID());
                    txRoomInfo.setOwnerId(timGroupDetailInfo.getOwner());
                    txRoomInfo.setMemberCount(timGroupDetailInfo.getMemberCount());
                    txRoomInfo.setCoverUrl(timGroupDetailInfo.getFaceUrl());
                    txRoomInfo.setRoomName(timGroupDetailInfo.getGroupName());
                    ...
                    if (pair != null) {
                        ...
                        for (IMAnchorInfo anchor : list) {
                            if (anchor.userId.equals(txRoomInfo.getOwnerId())) {
                                txRoomInfo.setStreamUrl(anchor.streamId);
                                txRoomInfo.setOwnerName(anchor.name);
                                break;
                            }
                        }
                    }
    ...
    public void updateStreamId(String streamId, TXCallback callback) {
        ...
        if (isOwner()) {
            ...
            mTXRoomInfo.setStreamUrl(streamId);
            updateHostAnchorInfo();
        }
        if (callback != null) {
    ...
    public void handleAnchorEnter(String userId) {
        TRTCLogger.INSTANCE.i(TAG, "handleAnchorEnter roomStatus "
    ...
    public void handleAnchorExit(String userId) {
        TRTCLogger.INSTANCE.i(TAG, "handleAnchorExit roomStatus "
                + mInternalStatus + " " + userId + " pk " + mPKingIMAnchorInfo);
    ...
    public void getAudienceList(final TXUserListCallback callback) {
        if (!isEnterRoom()) {
            TRTCLogger.INSTANCE.e(TAG, "get audience info list fail, not enter room yet.");
            if (callback != null) {
                ...
                new V2TIMValueCallback<V2TIMGroupMemberInfoResult>() {
                    ...
                    public void onSuccess(V2TIMGroupMemberInfoResult v2TIMGroupMemberInfoResult) {
                        ...
                        for (V2TIMGroupMemberFullInfo info : v2TIMGroupMemberInfoResult.getMemberInfoList()) {
                            ...
                            userInfo.setUserName(info.getNickName());
                            userInfo.setUserId(info.getUserID());
                            userInfo.setAvatarURL(info.getFaceUrl());
                            list.add(userInfo);
                        }
                    ...
                    @Override
                    public void onError(int i, String s) {
                        TRTCLogger.INSTANCE.e(TAG, "getGroupMemberList fail, code: " + i + " msg:" + s);
                        if (callback != null) {
                            callback.onCallback(CODE_ERROR,
                                    s, new ArrayList<>());
                        }
    ...
    public void getUserInfo(final List<String> userList, final TXUserListCallback callback) {
        if (!isEnterRoom()) {
            TRTCLogger.INSTANCE.INSTANCE.e(TAG, "get user info list fail, not enter room yet.");
            if (callback != null) {
                callback.onCallback(CODE_ERROR,
                        "get user info list fail, not enter room yet.", new ArrayList<TXUserInfo>());
            }
        ...
        V2TIMManager.getInstance().getUsersInfo(userList, new V2TIMValueCallback<List<V2TIMUserFullInfo>>() {
            @Override
            public void onError(int i, String s) {
                TRTCLogger.INSTANCE.INSTANCE.e(TAG, "get user info list fail, code:" + i);
                if (callback != null) {
                    callback.onCallback(i, s, null);
                }
            }
            ...
            public void onSuccess(List<V2TIMUserFullInfo> v2TIMUserFullInfos) {
                ...
                if (v2TIMUserFullInfos != null && v2TIMUserFullInfos.size() != 0) {
                    for (int i = 0; i < v2TIMUserFullInfos.size(); i++) {
                        TXUserInfo userInfo = new TXUserInfo();
                        userInfo.setUserName(v2TIMUserFullInfos.get(i).getNickName());
                        userInfo.setUserId(v2TIMUserFullInfos.get(i).getUserID());
                        userInfo.setAvatarURL(v2TIMUserFullInfos.get(i).getFaceUrl());
                        list.add(userInfo);
                    }
                }
                if (callback != null) {
    ...
    public void sendRoomTextMsg(String msg, final TXCallback callback) {
        if (!isEnterRoom()) {
            TRTCLogger.INSTANCE.INSTANCE.e(TAG, "send room text fail, not enter room yet.");
            if (callback != null) {
        ...
        V2TIMManager.getInstance().sendGroupTextMessage(msg, mRoomId,
                V2TIMMessage.V2TIM_PRIORITY_LOW, new V2TIMValueCallback<V2TIMMessage>() {
                    @Override
                    public void onError(int i, String s) {
                        TRTCLogger.INSTANCE.INSTANCE.e(TAG, "message send fail, code: " + i + " msg:" + s);
                        if (callback != null) {
                            callback.onCallback(i, s);
                        }
                    }
    ...
    public void sendRoomCustomMsg(String cmd, String message, final TXCallback callback) {
        if (!isEnterRoom()) {
            TRTCLogger.INSTANCE.INSTANCE.e(TAG, "send room custom msg fail, not enter room yet.");
            if (callback != null) {
    ...
    public String requestJoinAnchor(String reason, int timeout, final TXCallback callback) {
        if (!isEnterRoom()) {
            TRTCLogger.INSTANCE.INSTANCE.e(TAG, "request join anchor fail, not enter room yet.");
            if (callback != null) {
        ...
        if (mCurrentRoomStatus == TRTCLiveRoomDef.ROOM_STATUS_PK) {
            TRTCLogger.INSTANCE.e(TAG, "response join anchor fail, mCurrentRoomStatus " + mCurrentRoomStatus);
            if (callback != null) {
        ...
        if (mInternalStatus != STATUS_NONE) {
            TRTCLogger.INSTANCE.e(TAG, "request join anchor fail, status :" + mInternalStatus);
            if (callback != null) {
        ...
        if (isOwner()) {
            TRTCLogger.INSTANCE.e(TAG, "request join anchor fail, you are the owner of current room, id:" + mRoomId);
            if (callback != null) {
        ...
        if (!TextUtils.isEmpty(mOwnerIMInfo.userId)) {
            ...
            TRTCLogger.INSTANCE.i(TAG, "send " + mOwnerIMInfo.userId + " json:" + json);
            ...
            TRTCLogger.INSTANCE.e(TAG, "request join anchor fail, can't find host anchor user id.");
            if (callback != null) {
    ...
    public void responseJoinAnchor(String userId, boolean agree, String reason) {
        if (!isEnterRoom()) {
            TRTCLogger.INSTANCE.e(TAG, "response join anchor fail, not enter room yet.");
            return;
        }
        if (mInternalStatus != STATUS_NONE
                && mInternalStatus != STATUS_RECEIVED) {
            TRTCLogger.INSTANCE.e(TAG, "response join anchor fail, roomStatus " + mInternalStatus);
            return;
        }
        String inviteID = mJoinAnchorUserIdMap.remove(userId);
        if (TextUtils.isEmpty(inviteID)) {
            TRTCLogger.INSTANCE.e(TAG, "inviteID is empty:" + userId);
            return;
        }
        if (isOwner()) {
            String json = createSignallingData(IMProtocol.SignallingDefine.CMD_REQUESTJOINANCHOR);
            ...
            TRTCLogger.INSTANCE.e(TAG, "send join anchor fail, not the room owner, room id:"
    ...
    public String kickoutJoinAnchor(String userId, final TXCallback callback) {
        if (!isEnterRoom()) {
            TRTCLogger.INSTANCE.e(TAG, "kickout join anchor fail, not enter room yet.");
            if (callback != null) {
        ...
        if (isOwner()) {
            if (mMySelfIMInfo.userId.equals(userId)) {
                TRTCLogger.INSTANCE.e(TAG, "kick out join anchor fail, you can't kick out yourself.");
                if (callback != null) {
            ...
            TRTCLogger.INSTANCE.e(TAG, "kick out anchor fail, not the room owner, room id:"
                    + mRoomId + " my id:" + mMySelfIMInfo.userId);
            if (callback != null) {
    ...
    public String requestRoomPK(String roomId, final String userId, int timeout, final TXCallback callback) {
        if (!isEnterRoom()) {
            TRTCLogger.INSTANCE.e(TAG, "request room pk fail, not enter room yet.");
            if (callback != null) {
        ...
        if (mCurrentRoomStatus == TRTCLiveRoomDef.ROOM_STATUS_LINK_MIC) {
            TRTCLogger.INSTANCE.e(TAG, "request room pk fail, room status is " + mInternalStatus);
            if (callback != null) {
        ...
        if (isOwner()) {
            ...
            TRTCLogger.INSTANCE.e(TAG, "request room pk fail, not the owner of current room, room id:" + mRoomId);
            if (callback != null) {
    ...
    public void responseRoomPK(String userId, boolean agree, String reason) {
        if (!isEnterRoom()) {
            TRTCLogger.INSTANCE.e(TAG, "response room pk fail, not enter room yet.");
            return;
        }
        if (mInternalStatus != STATUS_RECEIVED) {
            TRTCLogger.INSTANCE.e(TAG, "response room pk fail, roomStatus is " + mInternalStatus);
            return;
        }
        ...
        if (isOwner()) {
            ...
            TRTCLogger.INSTANCE.e(TAG, "response room pk fail, not the owner of this room, room id:" + mRoomId);
        }
    }
    ...
    private void changeRoomStatus(int status) {
        TRTCLogger.INSTANCE.e(TAG, "changeRoomStatus " + status);
    ...
    public String quitRoomPK(TXCallback callback) {
        if (!isEnterRoom()) {
            TRTCLogger.INSTANCE.e(TAG, "quit room pk fail, not enter room yet.");
            if (callback != null) {
        ...
        if (isOwner()) {
            ...
            if (pkingAnchorInfo != null && !TextUtils.isEmpty(pkingAnchorInfo.userId)
                ...
                TRTCLogger.INSTANCE.e(TAG, "quit room pk fail, not in pking, pk room id:"
            ...
            TRTCLogger.INSTANCE.e(TAG, "quit room pk fail, not the owner of this room, room id:" + mRoomId);
            if (callback != null) {
    ...
    private void updateRoomType(int type) {
        ...
        mTXRoomInfo.setRoomStatus(type);
    ...
    private void updateHostAnchorInfo() {
        ...
        TRTCLogger.INSTANCE.i(TAG, "start update anchor info, type:" + mCurrentRoomStatus + " list:" + mAnchorList.toString());
        List<String> groupList = new ArrayList<>(Arrays.asList(mRoomId));
        V2TIMManager.getGroupManager().getGroupsInfo(groupList, new V2TIMValueCallback<List<V2TIMGroupInfoResult>>() {
            @Override
            public void onError(int i, String s) {
            ...
            public void onSuccess(List<V2TIMGroupInfoResult> v2TIMGroupInfoResults) {
                if (v2TIMGroupInfoResults != null && v2TIMGroupInfoResults.size() == 1) {
                    ...
                    TRTCLogger.INSTANCE.i(TAG, String.format("updateHostAnchorInfo, GroupName=%s, Introduction=%s",
                            v2TIMGroupInfo.getGroupName(), v2TIMGroupInfo.getIntroduction()));
                    V2TIMManager.getGroupManager().setGroupInfo(v2TIMGroupInfo, new V2TIMCallback() {
                        @Override
                        public void onError(int code, String desc) {
                            TRTCLogger.INSTANCE.e(TAG, "updateHostAnchorInfo fail, code: " + code + " msg:" + desc);
                        }

                        @Override
                        public void onSuccess() {
                            TRTCLogger.INSTANCE.i(TAG, "updateHostAnchorInfo success");
                        }
                    });
                }
            }
    ...
    private void onRecvC2COrGroupCustomMessage(final TXUserInfo txUserInfo, byte[] customData) {
        ...
        TRTCLogger.INSTANCE.i(TAG, "im msg dump, sender id:" + txUserInfo.getUserId() + " customStr:" + customStr);
        if (!TextUtils.isEmpty(customStr)) {
            try {
                ...
                if (!version.equals(IMProtocol.Define.VALUE_PROTOCOL_VERSION)) {
                    TRTCLogger.INSTANCE.e(TAG, "protocol version is not match, ignore msg.");
                }
    ...
    private class LiveRoomSimpleMsgListener extends V2TIMSimpleMsgListener {
        ...
        public void onRecvC2CCustomMessage(String msgID, V2TIMUserInfo sender, byte[] customData) {
            ...
            txUserInfo.setUserId(sender.getUserID());
            txUserInfo.setUserName(sender.getNickName());
            txUserInfo.setAvatarURL(sender.getFaceUrl());
        ...
        public void onRecvGroupTextMessage(String msgID, String groupID, V2TIMGroupMemberInfo sender, String text) {
            ...
            txUserInfo.setUserId(sender.getUserID());
            txUserInfo.setUserName(sender.getUserID());
            txUserInfo.setAvatarURL(sender.getFaceUrl());
    ...
    private class LiveRoomGroupListener extends V2TIMGroupListener {
        ...
        public void onMemberEnter(String groupID, List<V2TIMGroupMemberInfo> memberList) {
            ...
            for (V2TIMGroupMemberInfo timUserProfile : memberList) {
                TXUserInfo userInfo = new TXUserInfo();
                userInfo.setUserName(timUserProfile.getNickName());
                Log.d(TAG, "onMemberEnter userName: " + userInfo.getUserName());
                userInfo.setUserId(timUserProfile.getUserID());
                userInfo.setAvatarURL(timUserProfile.getFaceUrl());
                mAudienceInfoMap.put(userInfo.getUserId(), userInfo);
                if (TextUtils.isEmpty(userInfo.getUserId()) || userInfo.getUserId().equals(mMySelfIMInfo.userId)) {
                    return;
                }
                if (mDelegate != null) {
        ...
        public void onMemberLeave(String groupID, V2TIMGroupMemberInfo member) {
            ...
            TXUserInfo userInfo = new TXUserInfo();
            userInfo.setUserName(member.getNickName());
            Log.d(TAG, "onMemberLeave userName: " + userInfo.getUserName());
            userInfo.setUserId(member.getUserID());
            userInfo.setAvatarURL(member.getFaceUrl());
            mAudienceInfoMap.remove(userInfo.getUserId());
            changeRoomStatus(STATUS_NONE);
            if (TextUtils.isEmpty(userInfo.getUserId()) || userInfo.getUserId().equals(mMySelfIMInfo.userId)) {
                return;
            }
        ...
        public void onGroupDismissed(String groupID, V2TIMGroupMemberInfo opUser) {
            TRTCLogger.INSTANCE.i(TAG, "recv room destroy msg");
            // If the room has been dismissed, exit the room internally
            exitRoom(new TXCallback() {
                @Override
                public void onCallback(int code, String msg) {
                    TRTCLogger.INSTANCE.i(TAG, "recv room destroy msg, exit room inner, code:" + code + " msg:" + msg);
        ...
        public void onGroupInfoChanged(String groupID, List<V2TIMGroupChangeInfo> changeInfos) {
            for (V2TIMGroupChangeInfo changeInfo : changeInfos) {
                ...
                TRTCLogger.INSTANCE.i(TAG, "onGroupInfoChanged change type: " + changeInfo.getType());
                if (V2TIMGroupChangeInfo.V2TIM_GROUP_INFO_CHANGE_TYPE_INTRODUCTION == changeInfo.getType()) {
                    String value = changeInfo.getValue();
                    TRTCLogger.INSTANCE.i(TAG, "onGroupInfoChanged changed introduction: " + value);
    ...
    private void parseGroupIntroduction(String introduction) {
        ...
        if (pair != null) {
            TRTCLogger.INSTANCE.i(TAG, "parse room info success, type:" + pair.first + " list:" + pair.second.toString());
            ...
            if (pair.second.size() > 0) {
                ...
                mTXRoomInfo.setOwnerName(ownerInfo.name);
                mTXRoomInfo.setOwnerId(ownerInfo.userId);
                mTXRoomInfo.setStreamUrl(ownerInfo.streamId);
            ...
            TRTCLogger.INSTANCE.e(TAG, "parse room info error, maybe something error.");
        }
    }
    ...
    private final String createSignallingData(String cmdType) {
        ...
        TRTCLogger.INSTANCE.i(TAG, "createSignallingData:" + json);
        return json;
    }
    ...
    private final class LiveV2TIMSignalingListener extends V2TIMSignalingListener {
        ...
        public void onReceiveNewInvitation(final String inviteID, final String inviter,
            ...
            TRTCLogger.INSTANCE.i(TAG, String.format("onReceiveNewInvitation enter, inviteID=%s, inviter=%s, groupID=%s, "
            ...
            if (IMProtocol.SignallingDefine.CMD_REQUESTJOINANCHOR.equals(cmd)) {
                if (mCurrentRoomStatus == TRTCLiveRoomDef.ROOM_STATUS_PK) {
                    TRTCLogger.INSTANCE.e(TAG, "recv join anchor mCurrentRoomStatus " + mCurrentRoomStatus);
                    // 主播正在PK中
                    rejectLinkMic(inviteID);
                    return;
                }
                if (mInternalStatus != STATUS_NONE) {
                    TRTCLogger.INSTANCE.e(TAG, "recv join anchor status " + mInternalStatus);
                ...
                if (mCurrentRoomStatus == TRTCLiveRoomDef.ROOM_STATUS_LINK_MIC) {
                    TRTCLogger.INSTANCE.e(TAG, "received pk msg, but mCurrentRoomStatus is" + mCurrentRoomStatus);
                ...
                if (mInternalStatus != STATUS_NONE) {
                    TRTCLogger.INSTANCE.e(TAG, "received pk msg, but roomStatus is" + mInternalStatus);
                    rejectRoomPk(inviteID);
                    return;
                }
                ...
                if (mDelegate != null) {
                    V2TIMManager.getInstance().getUsersInfo(Collections.singletonList(inviter),
                            new V2TIMValueCallback<List<V2TIMUserFullInfo>>() {
                                ...
                                public void onSuccess(List<V2TIMUserFullInfo> v2TIMUserFullInfos) {
                                    TXUserInfo userInfo = new TXUserInfo();
                                    userInfo.setUserId(inviter);
                                    userInfo.setUserName(v2TIMUserFullInfos.get(0).getNickName());
                                    mPKingIMAnchorInfo.name = userInfo.getUserName();
                                    mPKingIMAnchorInfo.userId = userInfo.getUserId();
        ...
        public void onInviteeAccepted(String inviteID, String invitee, String data) {
            TRTCLogger.INSTANCE.i(TAG, String.format("onInviteeAccepted enter, inviteID=%s, invitee=%s, data=%s",
        ...
        public void onInviteeRejected(final String inviteID, final String invitee, String data) {
            TRTCLogger.INSTANCE.i(TAG, String.format("onInviteeRejected enter, inviteID=%s, invitee=%s, data=%s",
                    inviteID, invitee, data));
            SignallingData signallingData = IMProtocol.convert2SignallingData(data);
            if (!IMProtocol.SignallingDefine.VALUE_BUSINESS_ID.equals(signallingData.getBusinessID())) {
                return;
            ...
            if (linkMicPair != null) {
                ...
                if (!TextUtils.isEmpty(inviteID0) && callback != null) {
                    if (inviteID0.equals(inviteID)) {
                        ...
                        TRTCLogger.INSTANCE.e(TAG, "recv join rsp, but link mic inviteID:" + inviteID0
                                + " recv inviteID:" + inviteID);
                    }
                } else {
                    TRTCLogger.INSTANCE.e(TAG, "recv join rsp, but link mic pair params is invalid, inviteID:"
                            + inviteID0 + " callback:" + callback);
                }
            } else {
                TRTCLogger.INSTANCE.e(TAG, "recv join rsp, but link mic pair is null.");
            }
        }
            ...
            if (pkPair != null) {
                ...
                if (!TextUtils.isEmpty(inviteID0)) {
                    if (inviteID0.equals(inviteID)) {
                        ...
                        TRTCLogger.INSTANCE.e(TAG, "recv pk rsp, but pk inviteID:" + inviteID0 + " im inviteID:" + inviteID);
                    }
                } else {
                    TRTCLogger.INSTANCE.e(TAG, "recv pk rsp, but pk pair params is invalid.");
                }
            } else {
                TRTCLogger.INSTANCE.e(TAG, "recv pk rsp, but pk pair is null.");
            }
        }

        @Override
        public void onInvitationCancelled(String inviteID, String inviter, String data) {
            TRTCLogger.INSTANCE.i(TAG, String.format("onInvitationCancelled enter, inviteID=%s, inviter=%s, data=%s",
        ...
        public void onInvitationTimeout(String inviteID, List<String> inviteeList) {
            TRTCLogger.INSTANCE.i(TAG, String.format("onInvitationTimeout enter, inviteID=%s, "
